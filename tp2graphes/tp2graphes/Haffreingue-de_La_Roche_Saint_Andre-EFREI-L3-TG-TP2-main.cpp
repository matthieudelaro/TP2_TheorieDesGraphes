...
int main(int argc, const char * argv[]) {
    ...
    //path = "/Users/matthieudelaro/Documents/classes/L3/S5/theorie_des_graphes/TP/tp2/github/tp2graphes/tp2graphes/Haffreingue-de_La_Roche_Saint_Andre-EFREI-L3-TG-TP2-2.txt";
...
bool connexe(AdjCell** adjMatrix, int size) {
    // Create two arrays the size of the matrix.
    // For each node i, found[i] and used[i]
    // store whether i has been found
    // and used to look for other nodes.
    vector<bool> found(size, false);
    vector<bool> used(size, false);
    
    // Each time we find a new node,
    // we incremente the value of foundQuantity.
    // This way, we know when we reach the end
    // of the algorithm (foundQuantity == size).
    // Idem for usedQuantity.
    int foundQuantity = 1;
    int usedQuantity = 0;
    found[0] = true;
    
    // We know we won't reach connexity when
    // we have not found some nodes while having
    // used every node, or when we have not found
    // any new node. So we need to store previous
    // value of foundQuantity.
    int previousFoundQuantity = 1;
    
    // We iterate over each line of the adjacency matrix.
    // If the node of the line has been found but not used yet,
    // we iterate over his neighboors. If they have not been found,
    // we add them as found nodes.
    // We may need to iterate over the whole adjacency matrix several times.
    int current = 0;
    bool goOn = true;
